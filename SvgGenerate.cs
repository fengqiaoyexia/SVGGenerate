...
namespace SingleToDuplciate
{
    public class SvgGenerate
    {
        ...
        public MemoryStream GenerateDuplicateIamgeStream()
        {
            ...
            var defsNode = root.SelectSingleNode("//svg:defs", _manager);
            if (defsNode != null)
            {
                UpdateDefsNode(defsNode);
            }

            var clipPathNode = root.SelectSingleNode("//svg:clipPath", _manager);
            if (clipPathNode != null)
            {
                UpdateClipPathNode(clipPathNode);
            }
        ...
        private void UpdateDefsNode(XmlNode node)
        {

        }

        private void UpdateClipPathNode(XmlNode node)
        {
        ...
        private XmlNode GenerateMaskChildNode(string id, double x, double y, double width, double height)
        {
            ...
            mask_rect.SetAttribute("width", height.ToString());
        ...
        private void AddMaskLayer(XmlNode node)
        {
            ...
            defs.AppendChild(GenerateMaskChildNode("originalMask", 530.5, 0, 361, 361));
            defs.AppendChild(GenerateMaskChildNode("duplicateMask", 69.5, 0, 361, 361));
